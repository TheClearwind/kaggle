...
class CentResnet(nn.Module):
    ...
    def __init__(self):
        ...
        self.feature = resnext50_32x4d(pretrained=False)
        self.shuffle = nn.PixelShuffle(4)
        self.conv1 = double_conv(128, 128)
        self.conv2 = double_conv(128, 128)
        self.conv3 = double_conv(128, 128)
        self.cls_conv = double_conv(128, 32)
        self.pose_conv = double_conv(128, 64)
        self.cls = conv1x1(32, 1)
        self.pose = conv1x1(64, 7)

    def forward(self, x):
        _, _, x = self.feature(x)
        x = self.shuffle(x)
        x = self.conv1(x)
        x = self.conv2(x)
        x = self.conv3(x)
        cls = self.cls_conv(x)
        cls = self.cls(cls)
        pose = self.pose_conv(x)
        pose = self.pose(pose)
        return torch.cat([cls, pose], 1)
...
if __name__ == '__main__':
    ...
    #
    # summary(CentResnet().to(device), (3, 512, 2048))
